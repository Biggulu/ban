...
const RESIDENTIAL_PATTERNS = [/家宽|家庭宽带|家庭住宅|住宅宽带|住宅|宽带/, /\bresi(?:dential)?\b/i, /\bhome(?:\s|-|_)?ip\b/i, /\bhome(?:\s|-|_)?broadband\b/i, /\bbroadband\b/i, /\bisp\b/i, ];
...
const REGION_DB = [{
	id: 'HK',
	kw: ['香港', 'hong kong', 'hongkong', 'hkg'],
	iso: ['HK']
}, {
	id: 'TW',
	kw: ['台湾', '台北', '台中', '高雄', '新北', '桃园', 'taiwan', 'taipei', 'taichung', 'kaohsiung', 'tpe', 'twn'],
	iso: ['TW']
}, {
	id: 'CN',
	kw: ['中国', '大陆', '国内', '中国大陆', 'china', 'mainland', '回国节点', '回国专线', '回国线路', '回国加速', '回国服务', '直连国内', '国内直连', '中转国内', '落地国内', '北京', '上海', '广州', '深圳', 'beijing', 'shanghai', 'guangzhou', 'shenzhen', '成都', '重庆', '杭州', '南京', '武汉', '天津', '苏州', '西安', '长沙', 'chengdu', 'chongqing', 'hangzhou', 'nanjing', 'wuhan', 'tianjin', 'suzhou', 'xian', 'changsha', '沈阳', '青岛', '郑州', '大连', '东莞', '宁波', '厦门', '济南', '无锡', '合肥', '昆明', '福州', '哈尔滨', '佛山', '长春', '石家庄', '太原', '南宁', '贵阳', '乌鲁木齐', '兰州', '海口', '银川', '西宁', '拉萨', '呼和浩特', '电信', '联通', '移动', '铁通', 'chinatelecom', 'chinaunicom', 'chinamobile', 'chn', 'pek', 'pkx', 'pvg', 'szx', 'ctu', 'ckg', 'hgh', 'nkg', 'wuh', 'tsn', 'syx', 'xiy', 'csx', 'kmg', 'hak', 'dlc', 'tao', 'she', 'hrb', 'cgo'],
	iso: ['CN']
}, {
	id: 'JP',
	kw: ['日本', '东京', '大阪', '横滨', '名古屋', '福冈', '札幌', '京都', '神户', '千叶', '埼玉', '仙台', '广岛', '冲绳', '那霸', 'japan', 'tokyo', 'osaka', 'yokohama', 'nagoya', 'fukuoka', 'sapporo', 'kyoto', 'kobe', 'chiba', 'sendai', 'hiroshima', 'okinawa', 'naha', 'jpn', 'nrt', 'hnd', 'kix', 'ngo', 'fuk', 'cts', 'oka'],
	iso: ['JP']
}, {
	id: 'KR',
	kw: ['韩国', '首尔', '釜山', '仁川', '大田', '大邱', '光州', '济州', 'korea', 'seoul', 'busan', 'incheon', 'daejeon', 'daegu', 'gwangju', 'jeju', 'kor', 'icn', 'gmp', 'pus'],
	iso: ['KR']
}, {
	id: 'SG',
...
const _compiledRegions = REGION_DB.map(function(region) {
	...
	for (var j = 0; j < region.kw.length; j++) {
		...
		if (_isChinese(kw)) {
			...
			matchers.push({
				type: 'en',
				regex: _getWordBoundaryRegex(kw, false)
			})
		}
	}
	return {
...
const REGION_ORDER = ['HK', 'TW', 'SG', 'JP', 'KR', 'APAC', 'US', 'EU', 'AMERICAS', 'AFRICA', 'GLOBAL'];
...
const MANUAL_ORDER = ['HK', 'TW', 'JP', 'US', 'SG', 'KR', 'APAC', 'EU', 'AMERICAS', 'AFRICA', 'GLOBAL'];
...
function buildRegionPreferredProxies(primaryKey) {
	var order = [primaryKey].concat(REGION_ORDER.filter(function(key) {
...
function injectBusinessGroups(config, activeSmartNames) {
	...
	MANUAL_ORDER.forEach(function(key) {
		...
		var homeGroup = homeKeyName ? config['proxy-groups'].find(function(g) {
		...
		if (mergedProxies.length > 0) {
			var manualName = MANUAL[key];
			manualGroupNames.push(manualName);
			var manualGroup = {
				name: manualName,
				type: 'select',
			...
			var existIdx = config['proxy-groups'].findIndex(function(g) {
				return g && g.name === manualName
			});
			if (existIdx !== -1) {
				config['proxy-groups'][existIdx] = manualGroup
			} else {
				config['proxy-groups'].push(manualGroup)
			}
		}
	});
	...
	var streamHkProxies = insertManualProxies(filterActive(buildRegionPreferredProxies('HK')));
	var streamTwProxies = insertManualProxies(filterActive(buildRegionPreferredProxies('TW')));
	var streamJpProxies = insertManualProxies(filterActive(buildRegionPreferredProxies('JP')));
	var streamEuProxies = insertManualProxies(filterActive(buildRegionPreferredProxies('EU')));
	var directFirstProxies = insertManualProxies(filterActive(buildDirectFirstProxies()));
	var trackerProxies = insertManualProxies(filterActive(buildTrackerProxies()));
	var groups = [{
		name: BIZ.AI,
		type: 'select',
		proxies: aiProxies.slice()
	}, {
		name: BIZ.GPTAI,
		type: 'select',
		proxies: aiProxies.slice()
		...
		name: BIZ.CNMEDIA,
		type: 'select',
		proxies: directFirstProxies.slice()
	}, {
		name: BIZ.NFLX,
		type: 'select',
		proxies: standardProxies.slice()
		...
		name: BIZ.STREAM_HK,
		type: 'select',
		proxies: streamHkProxies.slice()
	}, {
		name: BIZ.STREAM_TW,
		type: 'select',
		proxies: streamTwProxies.slice()
	}, {
		name: BIZ.STREAM_JP,
		type: 'select',
		proxies: streamJpProxies.slice()
	}, {
		name: BIZ.STREAM_EU,
		type: 'select',
		proxies: streamEuProxies.slice()
	}, {
		...
		name: BIZ.AD,
		type: 'select',
		proxies: insertManualProxies(['REJECT', 'DIRECT', BIZ.GFW])
	}, {
		name: BIZ.SPEED,
		type: 'select',
		proxies: directFirstProxies.slice()
		...
		name: BIZ.WHITE,
		type: 'select',
		proxies: insertManualProxies(['DIRECT', 'REJECT', BIZ.GFW])
	}, {
		name: BIZ.BLACK,
		type: 'select',
		proxies: insertManualProxies(['REJECT', 'DIRECT', BIZ.GFW])
	...
	console.log(`[${VERSION}]Injected ${groups.length}business groups&${manualGroupNames.length}manual select groups`)
}
...
function injectRuleProviders(config) {
	...
	config['rule-providers']['goabroad'] = {
		type: 'http',
		behavior: 'classical',
		format: 'text',
		url: `${BAN}/Ruleset/GoAbroad.list`,
		path: './ruleset/goabroad.list',
		interval: nextInterval(),
		proxy: RP_PROXY
	};
	...
	config['rule-providers']['douyin-cn'] = {
		type: 'http',
		behavior: 'classical',
		format: 'text',
		url: `${BAN}/Ruleset/DY.list`,
		path: './ruleset/DY.list',
		interval: nextInterval(),
		proxy: RP_PROXY
	};
	...
	const AET = 'https://testingcf.jsdelivr.net/gh/Aethersailor/Custom_OpenClash_Rules@main/rule';
	...
	const SZKANE = 'https://fastly.jsdelivr.net/gh/szkane/ClashRuleSet@main/Clash';
...
function injectSmartFingerprint(config) {
	...
	config.proxies.forEach(p => {
		...
		} else if (/twitter|facebook|instagram|tiktok|snapchat|linkedin/i.test(name)) {
...
function main(config) {
	try {
		...
		if (c.HOME_JP.length > 0) upsertSmartGroup(config, SMART.JP_HOME, c.HOME_JP);
		if (c.KR.length > 0) upsertSmartGroup(config, SMART.KR, c.KR);
		if (c.HOME_KR.length > 0) upsertSmartGroup(config, SMART.KR_HOME, c.HOME_KR);
		...
		var activeSmartNames = new Set(config['proxy-groups'].filter(function(g) {
			return g && g.type === 'smart'
		}).map(function(g) {
			return g.name
		}));
